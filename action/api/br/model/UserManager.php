<?php
    ...
    class UserManager extends BaseManager{
        ...
        function register($account, $vCode, $regist_type, $md5Pwd){//register type:0 phone 1: email
            ...
            $code = 0;
			$emailFromUser = '卓维科技';
			$emailTitle = '蓝牙录音机账户激活';
			$emailCall = '&nbsp;&nbsp;&nbsp;&nbsp;尊敬的 '.$account.'， 您好：</br></br>';
			$emailHeader = '&nbsp;&nbsp;&nbsp;&nbsp;请点击下面的链接激活账号完成注册。（请在48小时内完成）</br>';
			$emailUrl = '';
			$emailFooter = '&nbsp;&nbsp;&nbsp;&nbsp;如果该链接无法点击，请直接拷贝以上链接到浏览器地址栏访问。</br></br>';
			$emailSign = '&nbsp;&nbsp;&nbsp;&nbsp;此邮件由卓维网系统自动发出，系统不接受回信，请勿直接回复。</br>谢谢';
        	if(!empty($result)){//用户信息以及存在
                ...
                if($user_status_tmp == $status_not_active){//未激活
					...
					$code = $this->get('common_result_success');//执行成功
                    $data['status'] = $this->get('user_status_actived');//已激活，请登录
                }
        	} else {//新用户
                $is_first_login = 0;
